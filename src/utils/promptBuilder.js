...
${options.isPremium ? (() => {
  ...
  return `
🎯 明日撮るべきもの
今日の写真とは絶対に違う被写体を1つ指定する。${tomorrowHints ? `以下の候補から1つ選ぶ（今日の写真と同じものは絶対に選ばない）:
${tomorrowHints}` : ''}
ルール:
- 「何を」「いつ」「どう撮るか」を1行で具体的に書く（例: 「明日の午前中、窓際に並んだ焼き立てのバゲットを湯気ごと撮ってください」）
- なぜその写真がフォロワーの反応を取れるかを1文で説明する
- 合計2行以内。${photoStyleHint}
- 「店内の様子」「商品」「お客様の笑顔」のような抽象的な提案は禁止。固有名詞・時間帯・状態を必ず含める`;
})() : ''}
━━━━━━━━━━━━━━━━━━━━━━━━
...
${tomorrowHints}` : ''}
ルール:
- 「何を」「いつ」「どう撮るか」を1行で具体的に書く（例: 「明日の夕方、使い込んだ道具を西日が当たる場所に置いて撮って」）
- なぜその写真がフォロワーの反応を取れるかを1文で説明する
- 合計2行以内
- 「店内の様子」「商品」「お客様の笑顔」のような抽象的な提案は禁止。固有名詞・時間帯・状態を必ず含める`;
})() : ''}
