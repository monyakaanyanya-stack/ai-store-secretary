...
function buildOutputFormat(hint, hashtagInstruction, postLength = null) {
  ...
  // ── 超短文（2行以内）モード ──
  // 想起の一言・来店の一文を独立パートにせず、本文2行に溶かし込む
  if (postLength === 'xshort' || postLength === '超短文') {
    return `【出力形式】
${labelWarning}

[ 案A：記憶に残る日常 ]
（本文2行以内——想起トリガーと来店トリガーを自然に溶かし込む）
${tagLine}
...
1. 本文2行以内（想起トリガー＋来店トリガーを本文に溶かし込む。独立パートにしない）
2. ハッシュタグ
${buildCategoryRules(store.category)}
【重要ルール】
- ⚠️ 本文は**2行以内**を最優先の絶対制限とする。3行以上は絶対禁止
- 想起の一言・来店の一文を別行にしない。本文の中に自然に含める
...
- 文章を書くな。作業中の独り言を書け。写真の内容を説明しない
- 店主が実際に言いそうな独り言にする（視点は常に店主側。客のフリをしない）
- 想起の一言も独り言の延長で書く。五感を入れてもいいが無理に入れない。「〜の香りと〜の甘さが混ざって」のようなAI定型文は禁止
...
その撮影が「なぜ反応が取れそうか」を1文で添える。合計3行以内・短く。${photoStyleHint}
例:
📸 次の撮影アイデア
...
被写体・タイミングを明確に書き、「なぜ反応が取れそうか」を1文で説明する。合計2行以内。${photoStyleHint}` : ''}
━━━━━━━━━━━━━━━━━━━━━━━━
