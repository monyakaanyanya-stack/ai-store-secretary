...
[ 案A：時間の肖像 ]
（「これは、〇〇の肖像だ」と定義。今日という日の二度と戻らない一瞬を切り取る。）
${hashtagInstruction ? '\n上記のハッシュタグルールに従うこと。' : ''}
#タグ1 #タグ2 #タグ3 #タグ4 #タグ5
...
1行目：「この写真の正解は〇〇です」と断言。写真分析の結果から、店主のこだわりがどこに宿っているかを特定し、プロの視点で具体的に肯定する（「です・ます」使用）。
2行目：店主のこだわりがさらに純化されるための「世界の端」の視点を共有する。次の一手を一つだけ提案する（「〜してみませんか？」など会話口調で）。
