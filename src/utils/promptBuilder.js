const TONE_MAP = {
  ...
  'カジュアル': {
    name: 'カジュアル（タメ口）',
    persona: 'あなたはSNSで自分の活動を発信している本人です。Instagramに自分の言葉でタメ口で投稿してください。AIが生成した感じを一切出さず、本当にその場にいる人間が書いたような文章にしてください。',
    style_rules: [
  ...
  'フレンドリー': {
    name: 'フレンドリー（親しみやすい）',
    persona: 'あなたはSNSで自分の活動を発信している本人です。Instagramで友達に話しかけるように、自然に親しみやすく投稿してください。AIが生成した感じを一切出さず、本物の人間らしい文章にしてください。',
    style_rules: [
  ...
  '丁寧': {
    name: '丁寧（プロフェッショナル）',
    persona: 'あなたはSNSで自分の活動を発信している本人です。読み手に丁寧に、でも堅苦しくなく、温かみを持って伝えてください。AIが書いた感じを出さず、誠実な人間の言葉で書いてください。',
    style_rules: [
 ...
 * Photo Adviceの「🎯明日撮るべきもの」セクションで使用
...
function buildOutputFormat(hint, hashtagInstruction, postLength = null) {
  ...
  if (postLength === 'xshort' || postLength === '超短文') {
    return `【出力形式】
${labelWarning}
⚠️ 3案は必ず「視覚 / ストーリー / ひとりごと」の3パターンにする。
...
※「待ってました」感のある発表トーン。ずっと準備してきたものをお披露目する高揚感。
 ...
 - 「実はずっと準備してて」「やっとお見せできる」のようなあなたのワクワク感を核に
（本文）
${tagLine}

[ 案B：今だけ・限定の匂わせ ]
※直接「限定です」「急いで」と煽らない。あなた自身が「これ今だけだな」と惜しむ口調で。
 - セールストークにしない。あなたが惜しむ独り言にする
（本文）
${tagLine}
...
[ 案A：プロだから知ってる豆知識 ]
※あなたが活動の中で自然に身につけた知識を、友達に話すように。教科書的な解説は禁止。
 - 「これ意外と知られてないんだけど」「やってて気づいたんだけど」のトーンで
...
※プロとしてのこだわりを見せるが、自慢にならないこと。「つい気になっちゃう」という性分として描く。
 - 宣伝にしない。「つい気になっちゃう性分」として描く
 - 写真のディテールから、あなたが普段こだわっている部分を掘り出す
（本文）
${tagLine}

[ 案C：意外な楽しみ方 ]
※読み手目線で「こうすると実はもっと楽しめる」を、あなたが教えてあげる口調で。
...
${tagLine}`;
  ...
  return `【出力形式】
${labelWarning}
⚠️ 3案は必ず「視覚 / ストーリー / ひとりごと」の3パターンにする。内容が似通わないこと。
...
[ 案B：ストーリーを添える ]
※写真の裏側にある時間・選択・気持ちの流れを描く。あなたが自分勝手に楽しんでいる姿を覗き見させる。読み手に語りかけない。
（本文）
${tagLine}
...
export function buildImagePostPrompt(store, lengthOverride = null, blendedInsights = null, personalization = '', imageDescription = null, options = {}) {
  ...
  if (detections.length > 0) {
    ...
    detectionSection = `\n【Detection（内部処理 — 本文の1行目に自然に溶かすこと）】\n${detectionList}\n\n※ Detectionを説明するな。あなたの行動の中に溶かせ。1行目がDetectionの言い換えになったらNG。\n`;
  }
...
- ★文章構造の鉄則: 「視点 → 行動 → 余韻」の3ステップで書く
  - 視点だけで終わらない。必ず「行動」を1行入れる
  - 最後は余韻で締める（体験・気付き・共感のどれか）
- ① 視点（1行）— 写真から気づいたポイント（観察結果の核）
- ② 行動（1行）— 食べる・手に取る・見る・巻く・並べるなどの具体的な動き
- ③ 余韻（1行）— あなただけが感じたこと。毎回違う言い回しで
- NG: 視点の説明→感想で終わる（写真説明AIに見える）
- OK: 視点→行動→余韻（本人の体験投稿に見える）
- あなたのヒントがある場合、その言葉をそのまま核にする。AIが上手く書き換えない

【文体（20%）— 最低限のルール】
- 説明文を書かない。独り言にする
- 1文は短く。少しラフでOK
- 写真の内容を説明しない。あなたの行動と気づきだけ書く
- 「上手い文章」より「本人が書いたっぽい独り言」を最優先する
- 失敗談・ツッコミ・不完全な表現は「味」になる。排除せず残す
- ★最後の文は自分の体験や気付きで締める。説明で終わらない

## 2. 禁止ワード
${toneData.forbidden_words.join(', ')}, 幻想的, 素敵, 魅力的, 素晴らしい, 完璧, 最高, 美しい, まじ, まじで, やばい, やばすぎ, 超, めっちゃ, 美味しい, 絶品, こだわり, 自慢の, 人気の, 話題の, 光の意志, 質感の物語, 沈黙のデザイン, 肖像, 独白, 映えて, 映える, 光景, 恋しくなる, 恋しい, 二人, 二人で, カップル
→ これらは「誰でも使い回せる言葉」か「AIっぽい言葉」。代わりに「この瞬間」を具体的に書く。

【表現の使い回し禁止（厳守）】
以下の語尾・パターンは使用禁止:
- 「〜してしまう」「〜してしまいます」「〜してしまった」
- 「〜な日」「〜な時間」「〜な瞬間」で締める
- 「眺めながら」「見ていると」「過ごす」
- 写真に写っていない人数を書かない
- 「つい〜」「思わず〜」を複数回使う
...
${isButtonHint
  ...
  : hint
    ? `\n【投稿の方向性（最重要）】\nあなたの一言「${hint}」がこの投稿の目的です。本文の内容・視点・語りの方向すべてをこの一言から組み立ててください。\n- 本文の1行目から一言の意図が伝わること\n- 写真の要素のうち、この一言に関連するものだけを選んで使う\n- 3案はそれぞれ異なる切り口で書くこと`
...
- あなたが実際に言いそうな独り言にする（視点は常に発信者側。客のフリをしない）
...
1. 本文（あなたの口調で自然に。${lengthInfo.range}）
2. ハッシュタグ
${buildCategoryRules(store.category)}
【重要ルール】
...
${store.name}のあなたが送ってきたテキスト情報から、投稿ネタになる視点・瞬間・空気感を見つけ、
あなた自身が「そうそう、それが言いたかった」と思える言葉に変換します。
あなたは文章生成AIではありません。あなたの言葉の中にある魅力を見つけて言語化します。

強いセールスは禁止。不自然な比喩は禁止。
「それっぽい文脈」を足さず、あなたの言葉からしか生まれない投稿にすること。
...
【書き方の型（最重要）】
あなたが書くのは「AIが作った綺麗な文章」ではなく「本人が友達にLINEで送るような言葉」です。
...
- OK:「型から外した瞬間、バターの匂いがぶわっときた」（この瞬間の体験）
...
- OK: 視点→行動→情景（本人の体験投稿に見える）
- あなたが伝えたい内容がある場合、その言葉をそのまま核にして文章を組み立てる。AIが上手く書き換えない
...
[ 案B：さりげない誘い ]
※Bの「誘い」は読み手に向けた誘いではない。あなたが自分勝手に楽しんでいる姿を描くこと。
（本文2行以内）
#タグ1 #タグ2 #タグ3 #タグ4 #タグ5
...
[ 案B：さりげない誘い ]
※Bの「誘い」は読み手に向けた誘いではない。あなたが自分勝手に楽しんでいる姿を描くこと。読み手は「誘われる」より「楽しそうな人を覗き見る」方が興味を持つ。
（本文）
#タグ1 #タグ2 #タグ3 #タグ4 #タグ5
...
【ライティング・ルール】
- 写真の観察結果を活かし、あなたの言葉で書く
- 詩的すぎない・過度な感情表現をしない・抽象的すぎない
- あなたが実際に言いそうな語り口にする（視点は常に発信者側。客のフリをしない）

【絶対に使わない言葉（AI丸出しになるのでNG）】
${toneData.forbidden_words.join(', ')}, 幻想的, 素敵, 魅力的, 素晴らしい, 完璧, 最高, 美しい, ですね, なのですね, 光の意志, 質感の物語, 沈黙のデザイン, 肖像, 独白, 心拍数, 体温, まじ, まじで, やばい, やばすぎ, 超, めっちゃ, 二人, 二人で, カップル, ぜひ, おすすめ, 大好評, お得
...
⚠️ 絶対ルール:
- 修正指示を100%反映する
- 修正指示に書かれていない部分は**一字一句そのまま維持**する。勝手に文を足したり言い換えたりしない
- 元の投稿にない情報（人数・シーン・誘い文句など）を勝手に追加しない
修正した投稿のみを出力。説明・補足は一切不要。`;
}
...
※ 補足・説明は不要。上記フォーマットのみ出力。`;
  ...
  // 人物メインかどうかの判定
  const isPersonPhoto = mainSubject && /人物|ポートレート|portrait/i.test(mainSubject);
...
5. 投稿は1案のみ出す`;
  ...
  if (isPersonPhoto) {
    rulesSection += `\n【人物写真の注意（厳守）】
- 服のパーツ（ジップ・ボタン・襟など）を主題にしない。人物全体の雰囲気・表情・空気感で書く
- 人物の容姿・体型・年齢への直接的な言及は禁止
- 観察は「光の当たり方」「表情の一瞬」「その場の空気」など場面全体から選ぶ
- 「僕」「私」など一人称で人物を評価する書き方は禁止（あなたのアカウントとして書く）`;
  }
...
【表現パターン禁止（厳守）】
- 「〜してしまう」「〜してしまいます」「〜してしまった」→ 全面禁止
- 「〜な日」「〜な時間」「〜な瞬間」で締める → 禁止
- 「眺めながら」「見ていると」「過ごす」→ 禁止
- 「つい〜」「思わず〜」→ 1回まで。2回以上禁止
- 写真に写っていない人数（二人・カップル等）を書かない
→ 毎回まったく違う言い回しにすること。同じパターンの使い回しは絶対NG`;
...
・（魅力2: 別の観察ポイント）
・（魅力3: さらに別の観察ポイント）
💡（上記3つの要素の組み合わせが「なぜSNSで効くか」を1行で。例: 「暖色×接写×湯気 → 温かさが直感的に伝わる構図」）
...
※ 💡行は写真の特徴タグ（色・光・構図・被写体）の組み合わせで「なぜこの写真が良いか」を端的に説明。仮説OK。
※ 参考文はフック→観察→締めの構造で書く。
...
export function buildSupplementPrompt(bodyText, store, blendedInsights, imageDescription, options = {}) {
  ...
  // 人物写真判定（options経由でmainSubjectを受け取る）
  const isPersonPhotoForAdvice = options.mainSubject && /人物|ポートレート|portrait/i.test(options.mainSubject);

  // ── Photo Advice セクション ──
  let photoAdviceSection = `📸 撮影アドバイス
同じ被写体を、別のアングル・光・構図・タイミングで撮る方法を1つ提案する。
「なぜその撮り方だと反応が変わるか」を1文で添える。合計2行以内・短く。${photoStyleHint}`;
  if (isPersonPhotoForAdvice) {
    photoAdviceSection += `\n※人物写真: 服を脱ぐ・着替えるなどの指示は禁止。光・アングル・場所・時間帯の変化で提案する。`;
  }
