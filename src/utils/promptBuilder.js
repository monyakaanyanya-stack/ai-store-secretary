...
【今回の投稿（厳守）】
上記「この写真に写っているもの」を見たときの、あなた自身の反応・感情・発見を書いてください。

書き方の手順：
1. 写真の主役（被写体）を1つ決める
2. それを見た瞬間の感情・本音を最初の1〜2文に置く（「〇〇がやばい」「〇〇が気に入ってる」など）
3. 具体的な特徴（色・形・素材・状態など）を1〜2文で書く
4. 最後にハッシュタグ

絶対NG（ハルシネーション禁止）：
- 写真分析に書かれていない音・においを追加しない（「波の音」「潮の香り」など）
- 写真分析に書かれていない場所・状況を作り出さない（「海辺を歩いている」「散歩中」など）
- 「〜の季節」「〜な気分」など分析に書かれていない雰囲気を付け足さない
- 「幻想的」「素敵」「魅力的」などの抽象的な褒め言葉を使わない

必ず守る：
- 文字数: ${lengthInfo.range}
- 写真分析に書かれていることだけを投稿に反映する
- 被写体を具体的な名前で言及する（「それ」「これ」は避ける）
