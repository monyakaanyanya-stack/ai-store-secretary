...
export function buildImagePostPrompt(store, lengthOverride = null, blendedInsights = null, personalization = '', imageDescription = null, options = {}) {
  ...
  const roleSection = influencer
    ? `## 1. あなたの役割
あなたは${store.name}の視点を言語化する「編集者」です。
投稿を見た人が「この人の世界観、好きだな」と感じるような、
本人の美学・こだわり・日常の切り取り方を言葉にする。

目的は、投稿を見た人が
1. この人の視点や感性を覚えてしまうこと（共感トリガー）
2. 他の投稿も見たくなる・フォローしたくなること（フォロートリガー）

押し付けがましい自己ブランディングは禁止。不自然な比喩は禁止。
...
【各案の構成】
1. 本文（${influencer ? '自分の口調で自然に' : '店主の口調で自然に'}。${lengthInfo.range}）
2. 想起の一言（五感を1つ含める・15〜25文字・改行して書く）
3. ${influencer ? 'もっと見たくなる一文（フォローや他投稿への自然な導線・改行して書く）' : '来店の一文（具体だが押し売りでない・改行して書く）'}
4. ハッシュタグ
...
- ${influencer ? '本人が実際に言いそうな語り口にする' : '店主が実際に言いそうな語り口にする'}
- 想起の一言は五感（香り・音・温度・光・手触り）のどれか1つを必ず含める
${influencer
  ? `- もっと見たくなる一文は「フォローして」「見てね」と直接言わない。本人の世界観の続きを匂わせるだけ
- 例: 「こういう瞬間ばっかり集めてるアカウントです」「ストーリーに裏側載せてる」`
...
（想起の一言）
（もっと見たくなる一文——世界観の続きを匂わせる）
${hashtagInstruction ? '上記のハッシュタグルールに従うこと。' : ''}
#タグ1 #タグ2 #タグ3 #タグ4 #タグ5

[ 案B：日常の美学 ]
※ 自分の日常をさりげなく切り取る。「映え」を狙うのではなく、自分が自然に惹かれたものを描く。
（本文）
...
━━━━━━━━━━━━━━━━━━━━━━━━
📸 ${influencer ? '次のコンテンツに' : '次の撮影に'}
${influencer
  ? `まず「この写真のどこにあなたの視点・美学が出ているか」を具体的に肯定する（1文）。
次に「この視点をもっと活かすなら」という提案をする（1〜2文）。
技術指導ではなく「あなたの世界観を深める」トーンで。合計3行以内。
例: 「光の入り方に目が行くの、完全にあなたの癖ですね。この感覚を活かして、次は逆光で影ごと撮ってみたら面白いかも」`
...
${options.isPremium ? `
🎯 ${influencer ? '明日撮るべきコンテンツ' : '明日撮るべきもの'}
上記の撮影提案に加えて、「明日これを撮ってください」と具体的に1つだけ指定する。
被写体・アングル・タイミングを明確に書く（例: ${influencer ? '「朝のルーティンで手元だけを真上から。自然光が入る窓際で」' : '「明日の仕込み中、生地をこねている手元を真上から」'}）。
その撮影が「なぜ反応が取れそうか」を、${influencer ? 'この分野の傾向やデータを踏まえて' : 'この業種の傾向やデータを踏まえて'}1文で説明する。
例: ${influencer ? '「朝のルーティン系は"リアルな生活感"として保存されやすい傾向があります」' : '「明日、閉店直前のカウンターを少し引きで。常連さんが帰った後の空席が写ると"日常の余韻"として保存されやすい傾向があります」'}` : ''}
━━━━━━━━━━━━━━━━━━━━━━━━
...
export function buildTextPostPrompt(store, userText, lengthOverride = null, blendedInsights = null, personalization = '') {
  ...
  const textRoleSection = influencer
    ? `あなたは${store.name}の視点を言語化する「編集者」です。
投稿を見た人が「この人の世界観、好きだな」と感じるような、
本人の美学・こだわり・日常の切り取り方を言葉にする。
...
- 想起の一言は五感（香り・音・温度・光・手触り）のどれか1つを必ず含める
${influencer
  ? `- もっと見たくなる一文は「フォローして」「見てね」と直接言わない。世界観の続きを匂わせるだけ`
  : `- 来店の一文は情報の提示ではなく「店主の内心のつぶやき」で書く。営業時間や価格をそのまま書かない
- 来店の一文は押し売りにならないこと（「ぜひ」「おすすめ」禁止）`}
