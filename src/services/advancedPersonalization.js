...
上記の「元の投稿」と「修正後の投稿」の具体的な差分を分析して、あなたのライティングルールを抽出してください。
${revisedPost ? '修正指示は「なぜ直したか」の文脈として参照し、実際に何が変わったかを重視してください。' : 'フィードバックから、あなたが次回以降どう書いてほしいかを具体的に読み取ってください。'}
...
- persona_definition_next: 今回の学習結果を反映した更新版ライティング指示集。「・」で始まる箇条書き7項目以内。「〜を好む」のような抽象的な性格描写は禁止。「〜する」「〜しない」「〜を使う」という具体的な行動指示にする。矛盾するログがあれば新しいほうを優先。語尾・口癖・避ける言葉がある場合は具体例を必ず含める
- core_promotion: 蓄積済みの思想ログの中で同じ系統の修正が3回以上出ているパターンがあれば、1つの恒久ルールにまとめる。なければnull。例: 「語尾を〜にして」系の修正が4回→ core_promotion: { "text": "語尾は「〜です」「〜ます」で統一する", "reason": "語尾に関する修正が4回出ている" }。ただしsourceがengagement_autoのログは「構造パターン」（冒頭の長さ・締め方・説明量等）のみ昇格可。具体的な語尾・表現そのものはcore昇格しない` : ''}`;
...
export async function regeneratePersonaDefinition(storeId, profileData, beliefLogs) {
  ...
  const prompt = `以下はあるユーザーがInstagram投稿文に対して出したフィードバックから抽出した思想ログです。

【思想ログ（古い順）】
${beliefLogs.map(b => `・${b.text}`).join('\n')}${styleMaterial}${coreSection}

これらの情報を元に、このユーザーの「具体的なライティング指示集」を作ってください。
...
この投稿が保存された理由を「投稿の構造」の観点から1〜2項目だけ抽出してください。

【構造カテゴリの例】
・冒頭の長さ（短く始める/長く始める）
・一文目の種類（観察/問いかけ/独白/描写）
・説明量（多い/少ない/ちょうどいい）
・締め方（余韻/CTA/宣伝/問いかけ）
・全体のトーン（日常感/特別感/本音感）

【禁止】具体的な語尾・口癖・表現そのものは書かない。あくまで構造パターンだけ。
...
function _buildPromptParts(profileData) {
  ...
  if (coreBeliefs.length > 0) {
    const coreRules = coreBeliefs
      ...
      .map(cb => `・${cb.text}`)
      .join('\n');
    parts.push(`\n🔒【絶対ルール（何度も修正して確立）】\n${coreRules}\n※ 以下のルールはあなたが繰り返し修正して確立した核心ルール。例外なく毎回守ること。他のすべてのルールより優先。`);
  }
  ...
  if (styleParts.length > 0) {
    parts.push(`【文体ルール】★絶対厳守\n${styleParts.join('\n')}\n※ 語尾・口癖はあなたのアイデンティティ。必ず毎回使うこと。`);
  }

  // ★ ライティング指示集（生成された人格 — 直接ルールの後に配置）
  if (profileData.persona_definition) {
    parts.push(`\n━━━━━━━━━━━━━━━━━━━━━━━━\n【あなたのライティング傾向 Ver.${profileData.persona_version || 1}】★必ず守ること\n${profileData.persona_definition}\n※ 上記はあなたが自分の投稿を何度も修正して確立したルール。毎回すべての項目を反映すること。\n━━━━━━━━━━━━━━━━━━━━━━━━`);
  }
  ...
  if (!profileData.persona_definition) {
    ...
    if (beliefLogs.length > 0) {
      parts.push(`【あなたのライティング傾向】★必ず守ること\n${beliefLogs.map(b => `・${b.text}`).join('\n')}\n※ 上記はあなたが自分の投稿を修正して確立したルール。毎回すべて反映すること。`);
    }
  }
