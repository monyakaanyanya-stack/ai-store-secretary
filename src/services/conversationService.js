...
■ 修正 & 学習（学習:）
- 「学習: ○○」 → 投稿がある場合は修正＋今後にも反映、投稿がない場合はスタイルを記憶
  例: 「学習: もっとカジュアルに」「学習: 絵文字を少なめに」「学習: 短くして」
  例: 「学習: α7C来たよ！まじ持ちやすくてやばい💫 #カメラ好き」（書き直し例を見せてもOK）
  ※指示でも書き直し例でも、AIが自動判断して修正＋学習します
...
■ エンゲージメント報告
- 「報告: いいね120, 保存15, コメント5」 → 投稿一覧が表示され、番号を選ぶと実績記録&AI学習
...
■ モード切替（AI投稿 ↔ そのまま投稿）
- 「モード切替」 → AI投稿モードとそのまま投稿モードを切り替え
- AI投稿モード（デフォルト）: 写真を送るとAIが3案の投稿文を自動生成
- そのまま投稿モード: AIを使わず、写真＋自分で書いたテキストをそのままInstagramに投稿
  ※ Standardプラン以上 + Instagram連携が必要
  使い方: 「モード切替」→「📷 そのまま投稿」を選択 → 写真を送る → テキストを入力 → 投稿

■ ストック（投稿の下書き保存・予約投稿）
- 「ストック」 → 保存済みストック一覧を表示
- A/B/C選択後に「ストック保存」→ 下書きとして保存
- 「予約: 12:00」→ 指定時刻に自動投稿（Instagram連携済みの場合）
- 「ストック投稿」→ 保存したストックを即時投稿
...
- 「学習:」の使い方を聞かれたら: 「学習: の後に指示や書き直し例を送ると、投稿があれば修正＋学習、なければスタイルを記憶します」と説明
